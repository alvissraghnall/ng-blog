...
export class HomeComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    this.category = this.route.snapshot.queryParamMap
      .get("category")
      ?.toUpperCase();
    let indexOfCat = Object.values(Category).indexOf(
      this.category as unknown as Category,
    );
    const catEnum = Object.keys(Category)[indexOfCat];
    console.log(catEnum, this.loading);

    this.postService
      .getPosts(Category[catEnum as keyof typeof Category])
